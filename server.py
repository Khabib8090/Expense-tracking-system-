...
from backendd import db_helper
# import db_helper
...
def add_or_update_expenses(expense_date:date, expenses:List[Expense]):
    db_helper.delete_expenses_data(expense_date)
    for expense in expenses:
        db_helper.insert_expenses_data(expense_date, expense.amount, expense.category, expense.notes)
    return {"Message": "Expense updated successfully"}  

@app.post("/analytics/")
def get_analytics(date_range:DateRange):
    data = db_helper.fetch_expenses_sumuary(date_range.start_date , date_range.end_date)
    if data is None:
        raise HTTPException (status_code = 500, detail = "Failed to retrieve Expenses !")
    print("Fetched data from db_helper:", data)
